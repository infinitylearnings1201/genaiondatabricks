...
# MAGIC ### Use the Model for Inference
# MAGIC
# MAGIC Inference for models that are registered with a Feature Store table are different than inference with a Spark DataFrame. For inference, we will use **feature engineering client's `.score_batch()` method**. This method takes **a model URI** and **DataFrame with a primary key**.
# MAGIC
# MAGIC > **How does the function know which feature table to use?** If you visit **Artifacts** section of registered model, you will see a **`data`** folder is registered with the model. Also, model file includes **`data: data/feature_store`** statement to define feature data.
# MAGIC
