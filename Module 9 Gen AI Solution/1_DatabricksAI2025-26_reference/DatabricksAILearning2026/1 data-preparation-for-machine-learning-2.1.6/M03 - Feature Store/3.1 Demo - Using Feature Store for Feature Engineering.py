...
dataset_path = f"/Volumes/mltraining/datasets/files/telco/telco-customer-churn.csv" # Full path
...
table_name = f"mltraining.telco.telco_customer_features"
...
# MAGIC %sql
# MAGIC ALTER TABLE mltraining.telco.telco_customer_features SET TBLPROPERTIES ('delta.columnMapping.mode' = 'name', 'delta.minReaderVersion' = '2', 'delta.minWriterVersion' = '5');
# MAGIC ALTER TABLE mltraining.telco.telco_customer_features DROP COLUMNS (tenure)
...
timestamp_v3 = spark.sql(f"DESCRIBE HISTORY mltraining.telco.telco_customer_features").orderBy("version").collect()[2].timestamp
print(timestamp_v3)
