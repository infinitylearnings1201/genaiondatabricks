...
feature_dataset = mlflow.data.load_delta(
    table_name=f"mltraining.telco.telco_customer_churn",
    name="telco"
)

display(feature_dataset)
...
# Select only numeric columns and cast them to float
numeric_cols = feature_data_pd.select_dtypes(include=["number"]).astype(float)

# Concatenate back the churn column
feature_data_pd = feature_data_pd.concat([numeric_cols, feature_data_pd[["churn"]]], axis=1)
...
X_all = feature_data_pd.drop(labels=target_col, axis=1, errors='ignore')
...
with mlflow.start_run(run_name="Model Developing Tracking with MLflow Lab") as run:
    ...
    model_name = f"mltrining.telco.churnmodel"
